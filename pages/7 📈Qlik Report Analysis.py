...
tab1,tab2,tab3=st.tabs(["W/O Spouse Analysis","Simple File Analysis","Analysis With Batch Removal"])
...
def read_file(file):
    ...
    # If not CSV, try reading it as Excel (XLSX)
    try:
...
with tab1:
    ...
    qlikData=st.file_uploader('Upload Report Only.', type=['xlsx','xls','xlsb','qvd'])
    ...
    if st.session_state['qlikfile']!=None and st.button("Execute Process"):
        ...
        df=pd.DataFrame()
        df['Data Type'] =['Total Cases','Cleared without deviation', 'Invalid CNIC','Residual Cases','GRM CNIC Cases']
        df['Data'] = [maindata.shape[0],clearedCNIC.shape[0],invalidCNIC.shape[0],cnicGRM.shape[0],cnicGRM.shape[0]+invalidCNIC.shape[0]]

        df.loc[6.5]=["",""]
        df.loc[9]=["Total Cleared",df.loc[1.0]["Data"]]
        df=df.sort_index()
        ...
        summaryDatafile='Output files/FinalSummaryALL WO '+qlikData.name[:-5]+datetime.today().strftime('%d-%m-%Y')+'.xlsx'
        ...
        GRM_DATA=invalidCNIC._append([cnicGRM,Holdland,hold3rd,DamagedGRM])

        my_bar.progress(95, text="!!! Sabrookaaaa...  !!!")
        sleep(2)
        
        outputDatafile='Output files/FinalDataALL WO '+qlikData.name[:-5]+datetime.today().strftime('%d-%m-%Y')+'.xlsx'
        with pd.ExcelWriter(outputDatafile) as writer:
            Level2Summary.to_excel(writer, sheet_name='Data Summary',index=False)
            GRM_DATA.to_excel(writer, sheet_name='GRM-DATA',index=False)
            holdHazardNoDup.to_excel(writer, sheet_name='Data in Hazardeous NO-DUP',index=False)
            ...
            clearedHazardNoDup.to_excel(writer, sheet_name='All - Cleared',index=False)
        ...
        my_bar.progress(100, text="!!! Download krlo !!!")
        sleep(2)
...
with tab2:
    ...
    if st.session_state['qlikfile']!=None and st.button("Execute Process"):
        ...
        my_bar.progress(100, text="!!! Muh kya dekh rahy download kro aur kato shabash  !!!")
        sleep(2)
...
with tab3:
    ...
    qlikData=c1.file_uploader('Upload Report', type=['xlsx','xls','xlsb'])
    st.session_state['qlikfile']=qlikData

    batchData=c2.file_uploader('Upload Batch Data', type=['xlsx','xls','xlsb'])
    st.session_state['batchfile']=batchData

    if st.session_state['qlikfile']!=None and st.session_state['batchfile']!=None and st.button("Execute Process"):
        ...
        totalBatches=pd.read_excel(st.session_state['batchfile'])
        maindata=pd.read_excel(st.session_state['qlikfile'])
        ...
        maindata=maindata[~maindata["DA_CNIC"].isin(totalBatches["cnic_or_other_id.1"].to_list())]
        st.markdown("Data after batches removal: "+str(maindata.shape[0]))
        ...
        # Deviation in CNIC
        clearedCNIC=validCNIC2[validCNIC2["CNIC Deviation"]=="Matched"]
        print(clearedCNIC.shape)
        holdCNIC=validCNIC2[validCNIC2["CNIC Deviation"]=="Not Matched"]
        print(holdCNIC.shape)
        ...
        # Damage assesment
        cleared3rd["UD_Damage CategorySTD"]=cleared3rd["UD_Damage Category"].str.replace("visible","Partially").replace("partially","Partially").replace("-","Fully").replace("1","Fully").replace("Kacha","Fully").replace("Pakka","Partially")
        cleared3rd["DA_Damage CategorySTD"]=cleared3rd["DA_Damage Category"].str.replace("collapsed","Fully").replace("washedAway","Fully").replace("visible","Partially").replace("intact","Partially")
        cleared3rd["DA TypeSTD"]=cleared3rd["DA Type"].str.replace("hybrid","Kacha").replace("other","Kacha").replace("kacha","Kacha").replace("pucca","Pakka")
        cleared3rd["UU TypeSTD"]=cleared3rd["UU Type"].str.replace("Thatch","Kacha").replace("Plinth (Concrete-Super structure \\n Brick in mud mortar)","Kacha").replace("Plinth (Concrete-Super structure mud)","Kacha").replace("2","Kacha").replace("partially","Pakka").replace("partially}]","Pakka")
        ...
        DamagedCleared=cleared3rd[~cleared3rd["Category"].isin(["GRM Category1",
                                                        "GRM Category2",
                                                        "GRM Category3",
                                                        "GRM Category4",
                                                        "GRM Category5",
                                                        "GRM Category6",
                                                        "GRM Category7","GRM Category8"])]
        print(DamagedCleared.shape)
        DamagedGRM=cleared3rd[cleared3rd["Category"].isin(["GRM Category1",
                                                        "GRM Category2",
                                                        "GRM Category3",
                                                        "GRM Category4",
                                                        "GRM Category5",
        ...
        def mapSummary(df1,df2):
            ipvise=df1[["DistrictMAP","DA_CNIC"]].groupby(["DistrictMAP"]).agg('count').reset_index()
            ipvise=ipvise.merge(df2[["DistrictMAP","DA_CNIC"]].groupby(["DistrictMAP"]).agg('count').reset_index(),on="DistrictMAP")
            ipvise.columns=["IpName","Total Case","Cleared"]
            ipvise["% cleared"]=ipvise["Cleared"]/ipvise["Total Case"]
            districtt=df1[["DistrictMAP","District","DA_CNIC"]].groupby(["DistrictMAP","District"]).agg('count').reset_index()
            districtt=districtt.merge(df2[["District","DA_CNIC"]].groupby(["District"]).agg('count').reset_index(),on="District")
            districtt.columns=["IpName","District","Total Case","Cleared"]
            districtt["% cleared"]=districtt["Cleared"]/districtt["Total Case"]
            districtt["IpName"]=""
            districtt.loc[0.5]=districtt.loc[0]
            districtt.loc[0]=ipvise.loc[0]
            districtt.loc[5.5]=ipvise.loc[1]
            districtt.loc[12.5]=ipvise.loc[2]
            districtt.loc[14.5]=ipvise.loc[3]
            districtt.loc[19.5]=ipvise.loc[4]
            districtt=districtt.sort_index()
            districtt=districtt[["IpName","District","Total Case","Cleared","% cleared"]]
        ...
        Damaged=Damaged._append(Damaged.sum(numeric_only=True), ignore_index=True)
        Damaged=Damaged.fillna("Total")
        Damaged=Damaged.sort_values("index")
        Damaged
        Damaged["MAP"]=Damaged["index"].map({value: key for key, value in CatMap.items()})
        Damaged=Damaged.join(Damaged['MAP'].str.split('_',expand=True).add_prefix('Data_'))
        Damaged=Damaged[["Data_0","Data_2","Data_1","Data_3","index","Category"]]
        Damaged.columns=["MIS Structure","MIS Damaged Category","JV Structure","JV Damaged Category","Marked as","Total"]
        ...
        Level1Summary=pd.DataFrame.from_dict({
            ...
            df3rd[df3rd["Data Type"]=="Reconstruction started - Yes"]["Data"].item()
                                            +
            landGRM[landGRM["index"]=="Total"]["landownership_type"].item()
                                            +
            df[df["Data Type"]=="GRM CNIC Cases"]["Data"].item()
        }.items())
        ...
        summaryDatafile='Output files/FinalSummaryALL '+datetime.today().strftime('%d-%m-%Y')+'.xlsx'
        with pd.ExcelWriter(summaryDatafile) as writer:
            df.to_excel(writer, sheet_name='CNIC Summary',index=False)
            landALL.to_excel(writer, sheet_name='Land Titles Summary',index=False)
            landClear.to_excel(writer, sheet_name='Land Titles Cleared Summary',index=False)
        ...
        holdHazardNoDup["AI Status"]="Hazardeous - GRM"
        clearedHazardNoDup["AI Status"]="All - Cleared"
        GRM_DATA=invalidCNIC.append([holdCNIC_holdName_holdSFname,Holdland,hold3rd,DamagedGRM])

        my_bar.progress(95, text="!!! Sabrookaaaa...  !!!")
        sleep(2)
        
        outputDatafile='Output files/FinalDataALL '+datetime.today().strftime('%d-%m-%Y')+'.xlsx'
        with pd.ExcelWriter(outputDatafile) as writer:
            Level2Summary.to_excel(writer, sheet_name='Data Summary',index=False)
            GRM_DATA.to_excel(writer, sheet_name='GRM-DATA',index=False)
            holdHazardNoDup.to_excel(writer, sheet_name='Data in Hazardeous NO-DUP',index=False)
